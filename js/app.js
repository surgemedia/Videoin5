...
function postError(error_text){
  $('.errors').append('<p><i class="fa fa-warning"></i>  '+error_text+'</p>');
}
...
function contactInfomation(){
  console.log(required('#step_5 #quote li','.required','Please enter your full name', "Please enter your email address or we can't send you your V5",'Please enter your contact number, in case there is issues'));
}
...
function fakePath(input,text_input){
  $(input).change(function(event) {
  var text = $(input).val().split('\\').pop();
  $(text_input).text(text);
  $(text_input).val(text);
  });
}

fakePath('#logo_file','#logo');
fakePath('#video_file','#video');
...
function checkExt(){
 ...
 var results = array.indexOf(ext, 1);
if(results < 0){
         $('.errors').append("<p><i class='fa fa-warning'></i>Hmm, That file isn't quite right, we suggest choosing a differnt file.</p>");
     }
		  ...
		  urls: "video_upload.php",
		   ...
		   uploadProgress: function(event, position, total, percentComplete) {  
			 ...
			 if(percentComplete > 95){
				 $( ".processing_load" ).addClass("display");
			 }
		   },  
...
//New function for style change  - Brad
function realtimechange(rt_input_id, tr_impact_id, css_items){
	var gen_rt_value = rt_input_id.val();
    $tr_impact_id.css(css_items, gen_rt_value, 'important');
}
